  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      ...
      <div className="mb-6 p-4 bg-gradient-to-r from-purple-900/30 to-pink-900/30 rounded-lg border border-purple-500/30">
        <div className="text-sm font-medium text-purple-300 mb-3">自我进化循环</div>
        <div className="flex items-center justify-between text-xs">
          <div className="flex flex-col items-center gap-1">
            <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center">
              <span className="text-purple-400">1</span>
            </div>
            <span className="text-slate-400">反思</span>
          </div>
          <div className="flex-1 h-px bg-purple-500/30 mx-2" />
          <div className="flex flex-col items-center gap-1">
            <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center">
              <span className="text-purple-400">2</span>
            </div>
            <span className="text-slate-400">规划</span>
          </div>
          <div className="flex-1 h-px bg-purple-500/30 mx-2" />
          <div className="flex flex-col items-center gap-1">
            <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center">
      ...
      {records.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-slate-400">暂无进化记录</p>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-white">进化周期 #{records.length - index}</div>
                    <div className="text-sm text-slate-400">{formatTime(record.timestamp)}</div>
                  </div>
                </div>
              </div>

              <div className="mt-3 pt-3 border-t border-slate-600/30">
                <div className="text-sm text-slate-300">{record.summary}</div>
              </div>

              <div className="mt-3 flex gap-4 text-xs">
                <div className="flex items-center gap-1">
                  <AlertCircle className="w-3 h-3 text-yellow-400" />
                  <span className="text-slate-400">发现缺口: {record.gaps}</span>
                </div>
                <div className="flex items-center gap-1">
                  <Lightbulb className="w-3 h-3 text-blue-400" />
                  <span className="text-slate-400">建议改进: {record.recommendations}</span>
                </div>
              </div>
            </div>
          ))}
