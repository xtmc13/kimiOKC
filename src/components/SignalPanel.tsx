      ...
      <div className="space-y-3">
        {signals.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-slate-400">暂无交易信号</p>
          ...
          signals.map((signal, index) => (
            ...
            >
              ...
              {signal.details && signal.details.length > 0 && (
                <div className="mt-3 pt-3 border-t border-current border-opacity-20">
                  <div className="text-xs text-gray-500 dark:text-slate-500 mb-2">各工具分析:</div>
                  <div className="space-y-1">
                    {signal.details.slice(0, 3).map((detail, i) => (
                      <div key={i} className="flex items-center justify-between text-xs">
                        <span className="text-gray-500 dark:text-slate-400">{detail.tool}</span>
                        <span className={detail.signal === 'BUY' ? 'text-green-500' : detail.signal === 'SELL' ? 'text-red-500' : 'text-gray-400 dark:text-slate-400'}>
                          {detail.signal}
                        </span>
                      </div>
                    ))}
